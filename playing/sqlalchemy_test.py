...
class DateTimeAsIsoTextComparator(Comparator):
    # cf. """CONVERT_TZ(STR_TO_DATE(LEFT({0}, 23), '%Y-%m-%dT%H:%i:%s.%f'), RIGHT({0},6), "+00:00")""".format(fieldname)
...
class DateTimeAsIsoText2(sqlalchemy.types.UserDefinedType):
    ...
    def column_expression(self, col):
        # applies specialfunc to create e.g. "SELECT specialfunc(mycol) AS mycol"
        print "COLUMN_EXPRESSION"
        return func.lower(col, type_=self)


class Task():
    # don't inherit from Base, because we don't want to define an actual table for Task.
    # It merely serves to include common fields and functions - but that works OK.
...
phq9table = Table('phq9', metadata,
    Column('_pk', Integer, Sequence('_pk_seq'), primary_key=True),
    Column('when_created', DateTimeAsIsoText),
    Column('q1', Integer)
)
enddate = dateutil.parser.parse("2013-04-30T00:26:03.400+05:00")
s = select([phq9table]).where(phq9table.c.when_created > enddate) # does LEXICAL comparison, so the timezone bit will be ignored
