...
// ... adapted for Titanium environment and extended by RNC
...
MersenneTwister.prototype.seedArray = function (vector) {
    ...
    for (k = N > vector.length ? N : vector.length; k > 0; k--) {
        s = this.mt[i - 1] ^ (this.mt[i - 1] >>> 30);

        this.mt[i] = (
            this.mt[i] ^ (((((s & 0xffff0000) >>> 16) * 1664525) << 16) +
    ...
    for (k = N - 1; k; k--) {
        s = this.mt[i - 1] ^ (this.mt[i - 1] >>> 30);
        this.mt[i] = (
            this.mt[i] ^ (((((s & 0xffff0000) >>> 16) * 1566083941) << 16) +
...
MersenneTwister.prototype.next = function (bits) {
    var y,
        k,
        mag01 = []; // RNC // new Array(0, MATRIX_A);
    ...
    if (this.mti >= N) {
        ...
        for (k = N - M; k < N - 1; k++) {
            y = (this.mt[k] & UPPER_MASK) | (this.mt[k + 1] & LOWER_MASK);
            this.mt[k] = this.mt[k + (M - N)] ^ (y >>> 1) ^ mag01[y & 1];
        }
...
MersenneTwister.prototype.int32 = function () { // RNC
    return this.next(32);
};
...
MersenneTwister.prototype.boolean = function () { // RNC
    return this.next(1) === 1;
};

MersenneTwister.prototype.int16 = function () { // RNC
    return this.next(16);
};

MersenneTwister.prototype.long = function () { // RNC
    // NOTE: ...
...
MersenneTwister.prototype.float = function () { // RNC
    return this.real();
};

MersenneTwister.prototype.byte = function () { // RNC
    return this.next(8);
};

MersenneTwister.prototype.double = function () { // RNC
    return this.long() / 70368744177664.0; // 2^46
};
 ...
 * @returns {number}
 */
function random() { // RNC modified
    return instance.rnd();
}

exports.MersenneTwister = MersenneTwister; // RNC
exports.instance = instance; // RNC
