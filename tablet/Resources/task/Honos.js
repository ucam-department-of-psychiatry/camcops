...
function Honos(patient_id) {
    taskcommon.BaseTask.call(this, patient_id); // call base constructor
}

lang.inheritPrototype(Honos, taskcommon.BaseTask);
lang.extendPrototype(Honos, {
    ...
    isComplete: function () {
        if (!taskcommon.isCompleteByPrefix(this, "q", 1, nquestions)) {
            return false;
        }
    ...
    getDetail: function () {
        ...
        return taskcommon.valueDetail(this, "honos_q", "_s", " ", "q", 1, 8) +
            L("honos_q8problemtype_s") + " " + this.q8problemtype + "\n" +
            L("honos_q8otherproblem_s") + " " + this.q8otherproblem + "\n" +
            taskcommon.valueDetail(this, "honos_q", "_s", " ", "q", 9,
                                   nquestions) +
            "\n" +
            this.getSummary() + "\n";
    },
    ...
    edit: function (readOnly) {
        ...
        function makepage(q) {
            var options = [],
                title = L('honos_title_prefix') + q,
                elements,
                pageTag = "",
                j;
            for (j = 0; j <= 4; ++j) {
                options.push(new KeyValuePair(L("honos_q" + q +
                                                "_option" + j), j));
            }
            options.push(new KeyValuePair(L("honos_option9"), 9)); // common to all
            elements = [
                { type: "QuestionText", text: L("honos_q" + q) },
                {
                    type: "QuestionMCQ",
                    field: "q" + q,
                    options: options
            ...
            if (q === 8) { // ugly :)
                ...
                elements.push(
                    {
                        type: "QuestionText",
                        text: L("honos_q8problemtype_prompt")
                    },
                    {
                        ...
                        field: "q8problemtype",
                        options: [
                            new KeyValuePair(L("honos_q8problemtype_option_a"), "A"),
                            new KeyValuePair(L("honos_q8problemtype_option_b"), "B"),
                            new KeyValuePair(L("honos_q8problemtype_option_c"), "C"),
                            new KeyValuePair(L("honos_q8problemtype_option_d"), "D"),
                            new KeyValuePair(L("honos_q8problemtype_option_e"), "E"),
                            new KeyValuePair(L("honos_q8problemtype_option_f"), "F"),
                            new KeyValuePair(L("honos_q8problemtype_option_g"), "G"),
                            new KeyValuePair(L("honos_q8problemtype_option_h"), "H"),
                            new KeyValuePair(L("honos_q8problemtype_option_i"), "I"),
                            new KeyValuePair(L("honos_q8problemtype_option_j"), "J")
                        ]
                    },
                    {
                        ...
                        variables: [
                            {
                                ...
                                prompt: L("honos_q8otherproblem_prompt"),
                                hint: L("honos_q8otherproblem_hint")
                            }
                        ]
                    }
                );
        ...
        pages = [
            self.getClinicianDetailsPage(), // Clinician info 3/3
            {
                ...
                elements: [
                    ...
                    { type: "QuestionText", text: L("honos_instructions") }
                ]
            }
        ];
