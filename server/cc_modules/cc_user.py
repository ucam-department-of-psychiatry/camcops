...
SECURITY_ACCOUNT_LOCKOUT_FIELDSPECS = [
    dict(name="user", cctype="USERNAME",  # NB not PK
...
class User:
    ...
    FIELDSPECS = [
        dict(name="user", cctype="USERNAME", pk=True,
             comment="User name"),
    ...
    def __init__(self,
        ...
        # logging.debug(
        #   "User constructor: username=" + username + ",
        #   create_if_not_exists=" + str(create_if_not_exists))
        exists_already = pls.db.fetch_object_from_db_by_pk(
            self, User.TABLENAME, User.FIELDS, username)  # last is the pk
    ...
    def save(self) -> None:
        ...
        already_exists = pls.db.does_row_exist(User.TABLENAME, "user",
                                               self.user)
...
def delete_user(username: str) -> str:
    ...
    pls.db.delete_by_field(User.TABLENAME, "user", user.user)
