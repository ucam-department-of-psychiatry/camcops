...
class Task(object):  # new-style classes inherit from (e.g.) object
        ...
        if self.is_anonymous():
            ...
            self._patient = \
                cc_patient.get_current_version_of_patient_by_client_info(
                    self._device, self.patient_id, self._era)
    ...
    def manually_erase(self, username):
        ...
        self.erase_subtable_records_even_noncurrent(username)
    ...
    def get_task_pks_wc_for_patient_deletion(cls, which_idnum, idnum_value):
        ...
        query = """
            ...
            INNER JOIN patient
                ...
                AND {table}._device = patient._device
    ...
    def get_session_candidate_task_pks_whencreated(cls, session, sort=False,
        ...
        if session.restricted_to_viewing_user() is not None:
            wheres.append("{}._adding_user = ?".format(table))
            args.append(session.restricted_to_viewing_user())
        if session.filter_device is not None:
            wheres.append("{}._device = ?".format(table))
            args.append(session.filter_device)
    ...
    def get_office_html(self):
        ...
        """.format(
            ...
            adding_user=ws.webify(self._adding_user),
            server_pk=self._pk,
