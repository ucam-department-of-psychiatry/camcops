...
- **Record:** for our purposes a record corresponds to a patient. REDCap has no
  internal concept of a patient. A record will have multiple instruments, some
  of which may be "repeating", allowing the same form to be submitted at
  different times.

We use an XML fieldmap to describe how the rows in CamCOPS task tables are
translated into REDCap records. See :ref:`<redcap>`.
...
class RedcapUploader(object):
    ...
    def upload_record(self, record: Dict, **kwargs) -> Any:
        try:
