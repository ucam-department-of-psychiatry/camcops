...
"""
camcops_server/alembic/versions/0067_cpft_research_preferences.py
...
Revision ID: 0067
Revises: 0066
Creation date: 2021-07-20 13:07:35.320726
...
def upgrade():
    op.create_table(
        "cpft_research_preferences",
        sa.Column("contact_preference", sa.String(length=1), nullable=True),
        sa.Column("contact_by_email", sa.Boolean(), nullable=True),
        sa.Column("research_opt_out", sa.Boolean(), nullable=True),
        sa.Column(
            "patient_id",
            sa.Integer(),
            nullable=False,
        ...
        sa.ForeignKeyConstraint(
            ["_adding_user_id"],
            ["_security_users.id"],
            name=op.f("fk_cpft_research_preferences__adding_user_id"),
        ),
        sa.ForeignKeyConstraint(
            ["_device_id"],
            ["_security_devices.id"],
            name=op.f("fk_cpft_research_preferences__device_id"),
            use_alter=True,
        ),
        sa.ForeignKeyConstraint(
            ["_group_id"],
            ["_security_groups.id"],
            name=op.f("fk_cpft_research_preferences__group_id"),
        ),
        sa.ForeignKeyConstraint(
            ["_manually_erasing_user_id"],
            ["_security_users.id"],
        ...
        sa.ForeignKeyConstraint(
            ["_preserving_user_id"],
            ["_security_users.id"],
            name=op.f("fk_cpft_research_preferences__preserving_user_id"),
        ),
        sa.ForeignKeyConstraint(
            ["_removing_user_id"],
            ["_security_users.id"],
    ...
    with op.batch_alter_table(
        ...
        batch_op.create_index(
            batch_op.f("ix_cpft_research_preferences__device_id"),
            ["_device_id"],
            unique=False,
        )
        batch_op.create_index(
        ...
        batch_op.create_index(
            batch_op.f("ix_cpft_research_preferences_patient_id"),
            ["patient_id"],
            unique=False,
        )
        batch_op.create_index(
...
# noinspection PyPep8,PyTypeChecker
def downgrade():
    op.drop_table("cpft_research_preferences")
