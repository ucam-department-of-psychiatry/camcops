...
"""
camcops_server/alembic/versions/0039_khandaker_2_mojosociodemographics.py
...
def upgrade():
    op.create_table(
        ...
        sa.Column('relationship_status', sa.Integer(), nullable=True, comment='0 Single, 1 Married / Civil partnership, 2 In steady relationship, 3 Divorced / separated, 4 Widowed'),
        sa.Column('education', sa.Integer(), nullable=True, comment='0 No qualifications, 1 GCSE/O levels, 2 A levels, 3 Vocational/college (B. Tecs/NVQs etc), 4 University / Professional Qualifications'),
        sa.Column('employment', sa.Integer(), nullable=True, comment='0 No unemployed, 1 No student, 2 Yes full time, 3 Yes part time, 4 Full time homemaker, 5 Self employed, 6 Not working for medical reasons, 7 Other (specify)'),
        sa.Column('other_employment', sa.UnicodeText(), nullable=True, comment='Other (specify)'),
        sa.Column('accommodation', sa.Integer(), nullable=True, comment='0 Own outright, 1 Own with mortgage, 2 Rent from local authority etc, 3 Rent from landlord (private), 4 Shared ownership (part rent, part mortgage)5 Live rent free, 6 Other (specify)'),
        sa.Column('other_accommodation', sa.UnicodeText(), nullable=True, comment='Other (specify)'),
        ...
        sa.ForeignKeyConstraint(['_adding_user_id'], ['_security_users.id'], name=op.f('fk_khandaker_2_mojosociodemographics__adding_user_id')),
        sa.ForeignKeyConstraint(['_device_id'], ['_security_devices.id'], name=op.f('fk_khandaker_2_mojosociodemographics__device_id')),
        sa.ForeignKeyConstraint(['_group_id'], ['_security_groups.id'], name=op.f('fk_khandaker_2_mojosociodemographics__group_id')),
        sa.ForeignKeyConstraint(['_manually_erasing_user_id'], ['_security_users.id'], name=op.f('fk_khandaker_2_mojosociodemographics__manually_erasing_user_id')),
        sa.ForeignKeyConstraint(['_preserving_user_id'], ['_security_users.id'], name=op.f('fk_khandaker_2_mojosociodemographics__preserving_user_id')),
        sa.ForeignKeyConstraint(['_removing_user_id'], ['_security_users.id'], name=op.f('fk_khandaker_2_mojosociodemographics__removing_user_id')),
        sa.PrimaryKeyConstraint('_pk', name=op.f('pk_khandaker_2_mojosociodemographics')),
        mysql_charset='utf8mb4 COLLATE utf8mb4_unicode_ci',
        mysql_engine='InnoDB',
        mysql_row_format='DYNAMIC'
    )
    with op.batch_alter_table('khandaker_2_mojosociodemographics', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics__current'), ['_current'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics__device_id'), ['_device_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics__era'), ['_era'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics__group_id'), ['_group_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics__pk'), ['_pk'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics_id'), ['id'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics_patient_id'), ['patient_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_khandaker_2_mojosociodemographics_when_last_modified'), ['when_last_modified'], unique=False)


# noinspection PyPep8,PyTypeChecker
def downgrade():
    op.drop_table('khandaker_2_mojosociodemographics')
