"""
camcops_server/alembic/versions/0088_pgic.py
...
Revision ID: 0088
Revises: 0087
Creation date: 2025-10-03 15:13:18.149358
...
def upgrade() -> None: # a lot of columns for metadata beyond PGIC itself
    op.create_table('pgic',
    sa.Column('question', sa.Integer(), nullable=True, comment='1 Very Much Improved to 7 Very Much Worse'),
    ...
    sa.ForeignKeyConstraint(['_adding_user_id'], ['_security_users.id'], name=op.f('fk_pgic__adding_user_id')),
    sa.ForeignKeyConstraint(['_device_id'], ['_security_devices.id'], name=op.f('fk_pgic__device_id'), use_alter=True),
    sa.ForeignKeyConstraint(['_group_id'], ['_security_groups.id'], name=op.f('fk_pgic__group_id')),
    sa.ForeignKeyConstraint(['_manually_erasing_user_id'], ['_security_users.id'], name=op.f('fk_pgic__manually_erasing_user_id')),
    sa.ForeignKeyConstraint(['_preserving_user_id'], ['_security_users.id'], name=op.f('fk_pgic__preserving_user_id')),
    sa.ForeignKeyConstraint(['_removing_user_id'], ['_security_users.id'], name=op.f('fk_pgic__removing_user_id')),
    sa.PrimaryKeyConstraint('_pk', name=op.f('pk_pgic')),
    mysql_charset='utf8mb4 COLLATE utf8mb4_unicode_ci',
    mysql_engine='InnoDB',
    mysql_row_format='DYNAMIC'
    )
    with op.batch_alter_table('pgic', schema=None) as batch_op: #creates database indexes to make query doing faster
        batch_op.create_index(batch_op.f('ix_pgic__current'), ['_current'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic__device_id'), ['_device_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic__era'), ['_era'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic__group_id'), ['_group_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic__pk'), ['_pk'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic_id'), ['id'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic_patient_id'), ['patient_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_pgic_when_last_modified'), ['when_last_modified'], unique=False)
