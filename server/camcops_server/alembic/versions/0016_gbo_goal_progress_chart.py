...
"""
camcops_server/alembic/versions/0016_gbo_goal_progress_sheet.py
...
def upgrade():
    ...
    op.create_table('gbogpc',
        sa.Column('date', sa.Date(), nullable=True),
        sa.Column('session', sa.Integer(), nullable=True),
        sa.Column('goal_number', sa.Integer(), nullable=True),
        sa.Column('goal_description', sa.UnicodeText(), nullable=True),
        sa.Column('progress', sa.Integer(), nullable=True),
        sa.Column('whose_goal', sa.Integer(), nullable=True),
        sa.Column('whose_goal_other', sa.UnicodeText(), nullable=True),
        ...
        sa.ForeignKeyConstraint(['_adding_user_id'], ['_security_users.id'], name=op.f('fk_gbogpc__adding_user_id')),
        sa.ForeignKeyConstraint(['_device_id'], ['_security_devices.id'], name=op.f('fk_gbogpc__device_id')),
        sa.ForeignKeyConstraint(['_group_id'], ['_security_groups.id'], name=op.f('fk_gbogpc__group_id')),
        sa.ForeignKeyConstraint(['_manually_erasing_user_id'], ['_security_users.id'], name=op.f('fk_gbogpc__manually_erasing_user_id')),
        sa.ForeignKeyConstraint(['_preserving_user_id'], ['_security_users.id'], name=op.f('fk_gbogpc__preserving_user_id')),
        sa.ForeignKeyConstraint(['_removing_user_id'], ['_security_users.id'], name=op.f('fk_gbogpc__removing_user_id')),
        sa.PrimaryKeyConstraint('_pk', name=op.f('pk_gbogpc')),
        mysql_charset='utf8mb4 COLLATE utf8mb4_unicode_ci',
        mysql_engine='InnoDB',
        mysql_row_format='DYNAMIC'
    )
    with op.batch_alter_table('gbogpc', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_gbogpc__current'), ['_current'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc__device_id'), ['_device_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc__era'), ['_era'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc__group_id'), ['_group_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc__pk'), ['_pk'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc_id'), ['id'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc_patient_id'), ['patient_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_gbogpc_when_last_modified'), ['when_last_modified'], unique=False)

    # ### end Alembic commands ###


def downgrade():
    op.drop_table('gbogpc')
