    ...
    def get_task_html(self):
        ...
        h = u"""
            ...
            <table class="taskdetail">
                <tr><th width="60%">Question</th><th width="40%">Answer</th></tr>
        """.format(
...
class LshsLaroi2005(Task):
    ...
    @classmethod
    def get_tasklongname(cls):
        return u"Launay–Slade Hallucination Scale, revision of Larøi et al. (2005)"
    @classmethod
    def get_fieldspecs(cls):
    ...
    def get_task_html(self):
        ...
        ANSWER_DICT = { None: "?" }
        for option in range(0, 5):
            ANSWER_DICT[option] = str(option) + u" — " + WSTRING("lshs_laroi2005_option" + str(option))
        h = u"""
            <div class="summary">
                <table class="summary">
                    {}
        ...
        for q in range(1, LshsLaroi2005.NQUESTIONS + 1):
            h += u"""<tr><td>{}</td><td><b>{}</b></td></tr>""".format(
                "Q" + str(q) + u" – " + WSTRING("lshs_laroi2005_q" + str(q) + "_question"),
                get_from_dict(ANSWER_DICT, getattr(self, "q" + str(q)) )
            )
        h += u"""
            </table>
