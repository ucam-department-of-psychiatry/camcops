...
const LinkFunctionFamily LINK_FN_FAMILY_LOGIT(
    ...
    statsfunc::
        logitArray,  // link function; binomial()$linkfun; eta = logit(mu)
    statsfunc::
        logisticArray,  // inverse link function; binomial()$linkinv; mu = logistic(eta)
    statsfunc::
        derivativeOfLogisticArray,  // derivative of inverse link function; binomial()$mu.eta
    statsfunc::
        binomialVariance,  // variance function; binomial()$variance; V(mu) = mu * (1 - mu)
...
const LinkFunctionFamily LINK_FN_FAMILY_GAUSSIAN(
    ...
    statsfunc::
        identityArray,  // inverse link function; gaussian()$linkinv; mu = eta
    statsfunc::
        oneArray,  // derivative of inverse link function; gaussian()$mu.eta; y = x => dy/dx = y' = 1
...
const LinkFunctionFamily LINK_FN_FAMILY_POISSON(
    ...
    statsfunc::
        expArray,  // inverse link function; poisson()$linkinv; mu = exp(eta)
    statsfunc::
        expArray,  // derivative of inverse link function; poisson()$mu.eta; mu' = exp(eta)
    statsfunc::
        identityArray,  // variance function; poisson()$variance; V(mu) = mu
