...
void Glm::fitIRLSKaneLewis()
{
    ...
    for (m_n_iterations = 1; m_n_iterations <= m_max_iterations;
        ...
        // n,k * k,1  -> n,1
        const ArrayXXd g
            = family.inv_link_fn(eta);  // apply invlink to eta -> n,1
        ...
        const MatrixXd W
            = (gprime_squared / var_g).matrix().asDiagonal();  // n,n
        xold = x;
...
void Glm::fitIRLSSVDNewtonKaneLewis()
{
    ...
    ArrayXd t = ArrayXd::Zero(m
    );  // nobs,1  // NB confusing name choice, cf. R's t() for transpose
    ...
    for (m_n_iterations = 1; m_n_iterations <= m_max_iterations;
         ++m_n_iterations) {
        const ArrayXd t_good
            = subsetByElementBoolean(t, good);  // nobs_where_good,1
        const ArrayXd b_good
            = subsetByElementBoolean(b, good);  // nobs_where_good,1
        const ArrayXd weights_good
            = subsetByElementBoolean(weights, good);  // nobs_where_good,1
        ...
        const ArrayXd gprime
            = family.derivative_inv_link_fn(t_good);  // nobs_where_good,1
        if (gprime.isNaN().any()) {
            // As per original...
        ...
        ArrayXd to_z_good
            = t_good + (b_good - g) / gprime;  // nobs_where_ngood,1
        assignByBooleanSequentially(z, good, to_z_good);
        ArrayXd W_new_good
            = weights_good * (gprime.square() / varg);  // nobs_where_ngood,1
        ...
        const ArrayXXd S_u_good
            = subsetByRowBoolean(S_u, good);  // nobs_where_ngood,npred
        // Note that mat[boolvec] gives a 1-d result, whereas
        // mat[boolvec,] gives a 2-d result.
        const ArrayXd W_good
            = subsetByElementBoolean(W, good);  // nobs_where_ngood,1
        const ArrayXd z_good
            = subsetByElementBoolean(z, good);  // nobs_where_ngood,1
    ...
    const ArrayXd t_good
        = subsetByElementBoolean(t, good);  // nobs_where_good,1
    const MatrixXd S_u = S.matrixU();  // nobs,npred
    const MatrixXd S_u_good
        = subsetByRowBoolean(S_u, good);  // nobs_where_good,npred
    const MatrixXd S_v = S.matrixV();
    const MatrixXd x_possible = S_v
        * ((1 / S_d)
           * CROSSPROD(
                 S_u_good,
                 t_good
...
eigenfunc::IndexArray Glm::svdsubsel(const MatrixXd& A, Eigen::Index k)
{
    ...
    MatrixXd subsetted = subsetByColumnIndex(S.matrixV(), indexSeq(0, index_k))
                             .transpose();  // k,?
...
void Glm::fitIRLSRglmfit()
{
    ...
    */
    ...
    ArrayXd offset
        = ArrayXd::Zero(nobs);  // specifying it is not yet supported
