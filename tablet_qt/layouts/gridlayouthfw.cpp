...
QSize GridLayoutHfw::findSize(const GeomInfo& gi, int QLayoutStruct::*size)
    const
{
...
void GridLayoutHfw::addHfwData(GeomInfo& gi, QQGridBox* box, int width) const
{
    ...
    QLayoutStruct& ls
        = rdata[box->row];  // May have been influenced by OTHER items already
...
void GridLayoutHfw::distribute(const QRect& layout_rect)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        const int x2p
            = gi.m_col_data.at(c2).pos + gi.m_col_data.at(c2).size;  // x2+1
...
void GridLayoutHfw::setGeometry(const QRect& rect)
{
    ...
    if (parent_new_height != -1) {
        r.setHeight(
            parent_new_height - parent_margins.totalHeight()
        );  // change
    }
#endif
...
GridLayoutHfw::GeomInfo GridLayoutHfw::getGeomInfo() const
{
    ...
    if (gi.m_has_hfw) {
        gi.m_hfw_height
            = gi.m_size_hint.height();  // already incorporates extra
        gi.m_hfw_min_height
            = gi.m_min_size.height();  // already incorporates extra
    } else {
        gi.m_hfw_height = -1;
