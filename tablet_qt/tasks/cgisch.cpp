...
CgiSch::CgiSch(CamcopsApp& app, DatabaseManager& db, const int load_pk) :
    Task(
        app, db, CGISCH_TABLENAME, false, true, false
    )  // ... anon, clin, resp
{
    addFields(
        strseq(FP_SEVERITY, 1, NQ_PER_SECTION), QMetaType::fromType<int>()
    );
    addFields(
        strseq(FP_CHANGE, 1, NQ_PER_SECTION), QMetaType::fromType<int>()
    );
...
{
    return tr(
        "Clinician-administered; briefly rates illness severity and "
...
QStringList CgiSch::summary() const
{
    ...
    return QStringList{
        fieldSummary(
            strnum(FP_SEVERITY, 5), xstring("summary_i_5"), sep, suffix
        ),
        fieldSummary(
            strnum(FP_CHANGE, 5), xstring("summary_ii_5"), sep, suffix
        ),
    };
}
...
QStringList CgiSch::detail() const
{
    ...
    for (int i = 1; i <= NQ_PER_SECTION; ++i) {
        lines.append(
            fieldSummary(strnum(FP_SEVERITY, i), xstring(strnum("q", i), sep))
        );
    }
    lines.append("");
    ...
    for (int i = 1; i <= NQ_PER_SECTION; ++i) {
        lines.append(
            fieldSummary(strnum(FP_CHANGE, i), xstring(strnum("q", i), sep))
        );
    }
    return lines;
}
...
OpenableWidget* CgiSch::editor(const bool read_only)
{
    ...
    for (int i = 1; i <= 7; ++i) {
        severity_options.append(
            NameValuePair(xstring(strnum("i_option", i)), i)
        );
        change_options.append(NameValuePair(xstring(strnum("ii_option", i)), i)
        );
    }
    ...
    for (int i = 1; i <= NQ_PER_SECTION; ++i) {
        ...
        severity_qfields.append(
            QuestionWithOneField(question, fieldRef(strnum(FP_SEVERITY, i)))
        );
        change_qfields.append(
            QuestionWithOneField(question, fieldRef(strnum(FP_CHANGE, i)))
        );
    }
