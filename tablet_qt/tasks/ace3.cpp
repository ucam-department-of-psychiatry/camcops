...
const QString
    TAG_PG_LANG_COMMANDS_SENTENCES(QStringLiteral("pg_lang_commands_sentences")
    );
...
const QString
    FN_LANG_FOLLOW_CMD_PRACTICE(QStringLiteral("lang_follow_command_practice")
    );
const QString FP_LANG_FOLLOW_CMD(QStringLiteral("lang_follow_command"));
const int N_LANG_FOLLOW_CMD = 3;
const QString
    FP_LANG_WRITE_SENTENCES_POINT(QStringLiteral("lang_write_sentences_point")
    );
    ...
    AceFamily(app, db, ACE3_TABLENAME, parent)
{
    ...
    addField(
        FN_PICTURE1_BLOBID, QMetaType::fromType<int>()
    );  // FK to BLOB table
    addField(
        FN_PICTURE2_BLOBID, QMetaType::fromType<int>()
    );  // FK to BLOB table
    ...
    QuPagePtr page_attn(
        (new QuPage{
             ...
             new QuFlowContainer{
                 ...
                 (new QuMcq(
                      fieldRef(
                          FN_ATTN_NUM_REGISTRATION_TRIALS, false
                      ),  // not mandatory
