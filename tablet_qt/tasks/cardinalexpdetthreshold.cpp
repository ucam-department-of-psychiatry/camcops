...
CardinalExpDetThreshold::CardinalExpDetThreshold(
    ...
    Task(
        app, db, CARDINALEXPDETTHRESHOLD_TABLENAME, false, false, false
    )  // ... anon, clin, resp
{
    // Config
    ...
    addField(
        FN_BACKGROUND_FILENAME, QMetaType::fromType<QString>()
    );  // set automatically
    addField(
        FN_TARGET_FILENAME, QMetaType::fromType<QString>()
    );  // set automatically
    ...
    if (load_pk == dbconst::NONEXISTENT_PK) {
        ...
        setValue(
            FN_INITIAL_LARGE_INTENSITY_STEP,
            DEFAULT_INITIAL_LARGE_INTENSITY_STEP,
            false
        );
        setValue(
            FN_MAIN_SMALL_INTENSITY_STEP,
            DEFAULT_MAIN_SMALL_INTENSITY_STEP,
            false
        );
        setValue(
            FN_NUM_TRIALS_IN_MAIN_SEQUENCE,
            DEFAULT_NUM_TRIALS_IN_MAIN_SEQUENCE,
            false
        );
...
{
    return tr(
        "Rapid assessment of auditory/visual thresholds "
...
void CardinalExpDetThreshold::loadAllAncillary(const int pk)
{
    ...
    ancillaryfunc::loadAncillary<
        ...
        m_app,
        m_db,
        CardinalExpDetThresholdTrial::FN_FK_TO_TASK,
        order_by,
        pk
    );
}
...
QVector<DatabaseObjectPtr>
    CardinalExpDetThreshold::getAncillarySpecimens() const
{
    return QVector<DatabaseObjectPtr>{
        CardinalExpDetThresholdTrialPtr(
            new CardinalExpDetThresholdTrial(m_app, m_db)
        ),
    };
}
...
QStringList CardinalExpDetThreshold::summary() const
{
    return QStringList{
        ...
        QString("x75 [intensity for which p(detect) = 0.75]: <b>%1</b>")
            .arg(convert::prettyValue(x75(), DP)),
    };
}
...
OpenableWidget* CardinalExpDetThreshold::editor(const bool read_only)
{
    ...
    QuPagePtr page1(
        (new QuPage{
             boldtext(tr(
                 ...
                 "task, and DO NOT ALTER THEM in between runs or before "
                 "completing "
             ...
             text(tr(
                 "Then, make a note of the 75% (“x75”) threshold intensities "
                 "for "
                 "each stimulus, and start the Expectation–Detection task "
                 "(which "
    ...
    QuPagePtr page2(
        (new QuPage{
             ...
             mcq(FN_TARGET_NUMBER, target_options_auditory)
                 ->addTag(TAG_AUDITORY),
             mcq(FN_TARGET_NUMBER, target_options_visual)->addTag(TAG_VISUAL),
         })
            ->setTitle(TX_CONFIG_TITLE + " (2)")
            ->addTag(TAG_P2)
    );
             ...
             questionnairefunc::defaultGridRawPointer({
                 {ExpDetTextConst::configVisualTargetDurationS(),
                  new QuLineEditDouble(
                      fieldRef(FN_VISUAL_TARGET_DURATION_S), 0.1, 10.0
                  )},
                 {tr("Background intensity (usually 1.0)"),
                  new QuLineEditDouble(
                      fieldRef(FN_BACKGROUND_INTENSITY), zero, one
                  )},
                 {tr("Minimum starting intensity (e.g. 0.9)"),
                  new QuLineEditDouble(
                      fieldRef(FN_START_INTENSITY_MIN), zero, one
                  )},
                 {tr("Maximum starting intensity (e.g. 1.0)"),
                  new QuLineEditDouble(
                      fieldRef(FN_START_INTENSITY_MAX), zero, one
                  )},
                 {tr("Initial, large, intensity step (e.g. 0.1)"),
                  new QuLineEditDouble(
                      fieldRef(FN_INITIAL_LARGE_INTENSITY_STEP), zero, one
                  )},
                 {tr("Main, small, intensity step (e.g. 0.01)"),
                  new QuLineEditDouble(
                      fieldRef(FN_MAIN_SMALL_INTENSITY_STEP), zero, one
                  )},
                 {tr("Number of trials in the main test sequence (e.g. 14)"),
                  new QuLineEditInteger(
                      fieldRef(FN_NUM_TRIALS_IN_MAIN_SEQUENCE), 0, 100
                  )},
    ...
    connect(
        fieldRef(FN_MODALITY).data(),
        &FieldRef::valueChanged,
        this,
        &CardinalExpDetThreshold::validateQuestionnaire
    );
    connect(
        fieldRef(FN_START_INTENSITY_MIN).data(),
        &FieldRef::valueChanged,
        this,
        &CardinalExpDetThreshold::validateQuestionnaire
    );
    connect(
        fieldRef(FN_START_INTENSITY_MAX).data(),
        &FieldRef::valueChanged,
        this,
        &CardinalExpDetThreshold::validateQuestionnaire
    );
    ...
    connect(
        ...
        this,
        &CardinalExpDetThreshold::abort
    );
    connect(
        m_questionnaire.data(),
        &Questionnaire::completed,
        this,
        &CardinalExpDetThreshold::startTask
    );
...
void CardinalExpDetThreshold::validateQuestionnaire()
{
    ...
    m_questionnaire->setVisibleByTag(
        TAG_WARNING_MIN_MAX, duff_minmax, false, TAG_P3
    );
...
#define CONNECT_BUTTON(b, funcname)                                           \
    connect(                                                                  \
        ...
        &QPushButton::clicked,                                                \
        this,                                                                 \
        &CardinalExpDetThreshold::funcname,                                   \
        Qt::QueuedConnection                                                  \
    )
...
#define CONNECT_BUTTON_PARAM(b, funcname, param)                              \
    connect(                                                                  \
        ...
        &QPushButton::clicked,                                                \
        this,                                                                 \
        std::bind(&CardinalExpDetThreshold::funcname, this, param),           \
        Qt::QueuedConnection                                                  \
    )
...
qreal CardinalExpDetThreshold::getIntensity() const
{
    ...
    if (inInitialStepPhase()) {
        return prev->intensity()
            - valueDouble(FN_INITIAL_LARGE_INTENSITY_STEP);
    }
...
void CardinalExpDetThreshold::showVisualStimulus(
    const QString& filename_stem, const qreal intensity
)
{
...
void CardinalExpDetThreshold::startTask()
{
    ...
    connect(
        ...
        this,
        &CardinalExpDetThreshold::mediaStatusChangedBackground
    );
    timerfunc::makeSingleShotTimer(m_timer);

    // Prep the sounds
    if (auditory) {
        m_player_background->setSource(
            urlFromStem(valueString(FN_BACKGROUND_FILENAME))
        );
        soundfunc::setVolume(
            m_player_background, valueDouble(FN_BACKGROUND_INTENSITY)
        );
        m_player_target->setSource(urlFromStem(valueString(FN_TARGET_FILENAME))
        );
...
void CardinalExpDetThreshold::startTrial()
{
    ...
    if (present_target) {
        ...
        if (auditory) {
            ...
            showVisualStimulus(
                valueString(FN_TARGET_FILENAME), tr->intensity()
            );
        }
    } else {
        // Catch trial
    ...
    if (!auditory) {
        int stimulus_time_ms
            = msFromSec(valueDouble(FN_VISUAL_TARGET_DURATION_S));
...
void CardinalExpDetThreshold::offerChoice()
{
    ...
    ButtonAndProxy y = makeTextButton(
        m_scene, YES_BUTTON_RECT, BASE_BUTTON_CONFIG, TextConst::yes()
    );
    ButtonAndProxy n = makeTextButton(
        m_scene, NO_BUTTON_RECT, BASE_BUTTON_CONFIG, TextConst::no()
    );
...
void CardinalExpDetThreshold::recordChoice(const bool yes)
{
    ...
    if (!t.targetPresented() && yes) {
        ...
        m_trial_last_y_b4_first_n
            = getNBackNonCatchTrialIndex(1, m_current_trial);
        qDebug() << "First no response: m_trial_last_y_b4_first_n ="
                 << m_trial_last_y_b4_first_n;
    }
    clearScene();
    setTimeout(
        msFromSec(valueDouble(FN_ITI_S)), &CardinalExpDetThreshold::nextTrial
    );
}
