...
void initializeHonos65(TaskFactory& factory)
{
    static TaskRegistrar<Honos65> registered(factory);
}

Honos65::Honos65(CamcopsApp& app, DatabaseManager& db, const int load_pk) :
    Task(
        app, db, HONOS65_TABLENAME, false, true, false
    )  // ... anon, clin, resp
{
    addFields(
...
QString Honos65::shortname() const
{
    return "HoNOS 65+";
}
