...
protected:
    ...
    QString
        m_database_type;  // written only in constructor; thread-safe access
    ...
    QSqlDatabase
        m_db;  // connection owned by worker thread if m_threaded, else GUI thread
    ...
    QStringList
        m_created_tables;  // trivial internal helper; accessed only by GUI thread
