...
const QString KEYSPEC_ID_SHORT_DESCRIPTION(
    KEYPREFIX_ID_SHORT_DESCRIPTION + "%1"
);  // S->C
const QString KEYPREFIX_ID_VALIDATION_METHOD("idValidationMethod"
);  // S->C, new in v2.2.8
const QString KEYSPEC_ID_VALIDATION_METHOD(
    KEYPREFIX_ID_VALIDATION_METHOD + "%1"
);  // S->C, new in v2.2.8
    ...
    switch (m_upload_next_stage) {
        ...
        case NextUploadStage::CheckPoliciesThenStartUpload:
            ...
            if (!catalogueTablesForUpload(
                )) {  // checks per-table version requirements
                fail();
                return;
            }
...
void NetworkManager::requestRecordwisePkPrune()
{
    const QString sql
        = QString("SELECT %1, %2, %3 FROM %4")
              .arg(
                  delimit(dbconst::PK_FIELDNAME),  // result column 0
    ...
    dict[KEY_MOVE_OFF_TABLET_VALUES]
        = move_off_tablet_values.join(",");  // v2.3.0
