...
TaskSchedulePtrList CamcopsApp::getTaskSchedules()
{
    ...
    TaskSchedule specimen(
        *this, *m_sysdb, dbconst::NONEXISTENT_PK
...
)
{
    success = m_sysdb->decrypt(passphrase, true)
        && m_datadb->decrypt(passphrase, true) && m_sysdb->canReadDatabase()
...
void CamcopsApp::initGuiOne()
{
    ...
    if (all_screens.isEmpty()) {
        ...
        m_qt_logical_dpi.x
            = screen->logicalDotsPerInchX();  // can be e.g. 96.0126
        m_qt_logical_dpi.y
            = screen->logicalDotsPerInchY();  // can be e.g. 96.0126
...
void CamcopsApp::closeSubWindow()
{
    ...
    const int index
        = m_p_window_stack->addWidget(w);  // m_p_window_stack now owns w
    m_p_window_stack->setCurrentIndex(index);
...
{
    Patient specimen(
        *this, *m_datadb, dbconst::NONEXISTENT_PK
    );  // this is why function can't be const
    const WhereConditions where;  // but we don't specify any
    const SqlArgs sqlargs = specimen.fetchQuerySql(where);
...
int CamcopsApp::fontSizePt(uiconst::FontSize fontsize, const double factor_pct)
    const
{
    double factor;
    if (factor_pct <= 0) {
...
NetworkManager::UploadMethod CamcopsApp::getUploadMethodFromUser() const
{
    ...
    QAbstractButton* move
        = msgbox.addButton(tr("Move"), QMessageBox::AcceptRole);  // e.g. OK
    msgbox.addButton(
        TextConst::cancel(), QMessageBox::RejectRole
    );  // e.g. Cancel
    msgbox.exec();
