...
protected:
    ...
    QString
        m_key_doc;  // (If key event) Whisker document receiving the keypress
    bool m_is_client_message
        = false;  // Is this a message from another Whisker client?
    int m_client_message_source_clientnum
        = -1;  // (If client message) The sender's client number
