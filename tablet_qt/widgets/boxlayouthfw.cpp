...
4.  When the layout is activated, it first sets the fixed, minimum and/or       [RNC: QLayout::activate()]
    ...
    QLayout::sizeConstraint(), using the values calculated by
    ...
    each item and produce a final size constraint for the whole layout. This
    may or may not alter the current size of the widget.

5.  The layout is then asked to resize its contents to fit the current size of  [RNC: QLayoutPrivate::doResize()]
    the widget using QLayout::setGeometry(widget->size()). The specific
    implementation of the layout - whether it is a box layout, grid layout or
    something else then lays out its child items to fit this new size.
...
BoxLayoutHfw::BoxLayoutHfw(Direction dir, QWidget* parent) :
    ...
    m_dir(dir),
    m_spacing(-1),
    m_invalidate_due_to_self(false)
{
}
...
*/
BoxLayoutHfw::Margins BoxLayoutHfw::effectiveMargins(
        const BoxLayoutHfw::Margins& contents_margins) const
{
    int l = contents_margins.m_left;
    int t = contents_margins.m_top;
    int r = contents_margins.m_right;
    int b = contents_margins.m_bottom;
#ifdef Q_OS_MAC
    if (horz(m_dir)) {
...
BoxLayoutHfw::GeomInfo BoxLayoutHfw::getGeomInfo(const QRect& layout_rect) const
{
    ...
    QVector<QQLayoutStruct> a(n);  // RNC: extra Q
    ...
    for (int i = 0; i < n; i++) {
        ...
        QSize item_min = box->item->minimumSize();  // RNC: was just "min"
...
#ifdef Q_OS_MAC
                // RNC: alters spacing if there's a preceding widget, somehow.
        ...
        if (horz(m_dir)) {
            ...
            if (!ignore) {
                qqMaxExpCalc(maxh, verexp, dummy,  // RNC: extra q; alters first three parameters (max, exp, empty)
            ...
            // Widths
            if (!ignore) {
                qqMaxExpCalc(maxw, horexp, dummy,  // RNC: extra q; alters first three parameters (max, exp, empty)
    ...
    qDebug() << Q_FUNC_INFO
             ...
             << "(left margin" << effmarg.m_left
             << "top margin" << effmarg.m_top
             << "m_right_margin" << effmarg.m_right
             << "m_bottom_margin" << effmarg.m_bottom
             << "; m_dir" << m_dir << ")";
    for (int i = 0; i < n; ++i) {
        const QQLayoutStruct& ls = a.at(i);
        qDebug() << "... item" << i << ":"
                 << "stretch" << ls.stretch
...
{
    if (m_hfw_cache.contains(layout_width)) {
        return m_hfw_cache[layout_width];
    }

    Margins effmarg = getEffectiveMargins();
    HfwInfo hfwinfo;
    int w = effmarg.removeLeftRightMargins(layout_width);  // effective width
    ...
    if (!found) {
        gi = getGeomInfo(QRect(QPoint(0, 0),
                               QSize(layout_width, QLAYOUTSIZE_MAX)));
    }

    QVector<QQLayoutStruct>& a = gi.m_geom_array;
    int n = a.count();
    int h = 0;
    int mh = 0;
    ...
    hfwinfo.m_hfw_height = h;
    hfwinfo.m_hfw_min_height = mh;
#ifdef DEBUG_LAYOUT
    qDebug() << Q_FUNC_INFO
             << "For layout width" << layout_width
             << "(effective contents width" << w << "):"
             << "m_hfw_height" << hfwinfo.m_hfw_height
             << "m_hfw_min_height" << hfwinfo.m_hfw_min_height;
#endif
    m_hfw_cache[layout_width] = hfwinfo;
    return hfwinfo;
}
...
int BoxLayoutHfw::spacing() const
{
    if (m_spacing >=0) {
        ...
        return qqSmartSpacing(this, m_dir == LeftToRight || m_dir == RightToLeft
...
{
    GeomInfo gi = getGeomInfo(getCurrentMostLikelyRect());
...
QSize BoxLayoutHfw::minimumSize() const
{
    GeomInfo gi = getGeomInfo(getCurrentMostLikelyRect());
#ifdef DEBUG_LAYOUT
    qDebug() << Q_FUNC_INFO << "->" << gi.m_min_size;
#endif
...
int BoxLayoutHfw::heightForWidth(int w) const
{
    ...
    return hfw_info.hfw(getEffectiveMargins());
}
...
int BoxLayoutHfw::minimumHeightForWidth(int w) const
{
    ...
    HfwInfo hfw_info = getHfwInfo(w);
    return hfw_info.minhfw(getEffectiveMargins());
}
...
void BoxLayoutHfw::partialInvalidate()
{
#ifdef DEBUG_LAYOUT
    qDebug() << Q_FUNC_INFO;
#endif
    // Calling QLayout::invalidate() will also call our own invalidate().
    // So let it know:
    m_invalidate_due_to_self = true;
    QLayout::invalidate();  // will trigger parent widget resize if necessary
}


void BoxLayoutHfw::fullInvalidate()
{
#ifdef DEBUG_LAYOUT
    qDebug() << Q_FUNC_INFO;
#endif
...
void BoxLayoutHfw::setGeometry(const QRect& r)
{
...
#ifdef DEBUG_LAYOUT
        qDebug() << Q_FUNC_INFO << "nothing to do, for" << r;
#endif
        return;
    }

    if (!geometry_previously_calculated) {
        // The width has changed since we last called setGeom().
        // This means that our minimum height (etc.) may be wrong. So we need
        // to invalidate the layout.
#ifdef DEBUG_LAYOUT
        qDebug() << Q_FUNC_INFO << "triggering partial invalidation, for" << r;
#endif
        (void) getGeomInfo(r);  // will store it in the cache
        partialInvalidate();
        return;
    }
    ...
    QRect s = getContentsRect(r);

#ifdef DEBUG_LAYOUT
    qDebug() << Q_FUNC_INFO << "called with layout rect" << r
             << "; giving final rect for children" << s;
#endif
...
QRect BoxLayoutHfw::getContentsRect(const QRect& layout_rect) const
{
    ...
    Margins effmarg = getEffectiveMargins();
    QRect s(cr.x() + effmarg.m_left, cr.y() + effmarg.m_top,
            cr.width() - (effmarg.m_left + effmarg.m_right),
            cr.height() - (effmarg.m_top + effmarg.m_bottom));
...
QVector<QRect> BoxLayoutHfw::getChildRects(const QRect& contents_rect,
                                           const QVector<QQLayoutStruct>& a) const
{
...
void BoxLayoutHfw::insertSpacing(int index, int size)
{
    ...
    QLayoutItem* b;
    if (horz(m_dir)) {
        b = createSpacerItem(this, size, 0, QSizePolicy::Fixed, QSizePolicy::Minimum);
    } else {
        b = createSpacerItem(this, 0, size, QSizePolicy::Minimum, QSizePolicy::Fixed);
    }
...
void BoxLayoutHfw::insertStretch(int index, int stretch)
{
    ...
    QLayoutItem* b;
    if (horz(m_dir)) {
        b = createSpacerItem(this, 0, 0, QSizePolicy::Expanding, QSizePolicy::Minimum);
    } else {
        b = createSpacerItem(this, 0, 0, QSizePolicy::Minimum, QSizePolicy::Expanding);
    }
...
void BoxLayoutHfw::addStrut(int size)
{
    QLayoutItem* b;
    if (horz(m_dir)) {
        b = createSpacerItem(this, 0, size, QSizePolicy::Fixed, QSizePolicy::Minimum);
    } else {
        b = createSpacerItem(this, size, 0, QSizePolicy::Minimum, QSizePolicy::Fixed);
    }
...
/*
bool BoxLayoutHfw::updateParentGeometry() const
{
    if (QWidget* parent = parentWidget()) {
#ifdef DEBUG_LAYOUT
        qDebug() << Q_FUNC_INFO << "calling parent widget's updateGeometry()";
#endif
...
QRect BoxLayoutHfw::getCurrentMostLikelyRect() const
{
    if (QWidget* parent = parentWidget()) {
...
const BoxLayoutHfw::Margins& BoxLayoutHfw::getContentsMarginsAndCache() const
{
    if (m_contents_margins.isSet()) {
        return m_contents_margins;
    }
    int left, top, right, bottom;
    getContentsMargins(&left, &right, &top, &bottom);
    m_effective_margins = effectiveMargins(m_contents_margins);
    return m_contents_margins;
}


const BoxLayoutHfw::Margins& BoxLayoutHfw::getEffectiveMargins() const
{
    if (m_contents_margins.isSet()) {
        return m_effective_margins;
    }
    (void) getContentsMarginsAndCache();
    return m_effective_margins;
}


void BoxLayoutHfw::clearCaches()
{
    m_hfw_cache.clear();
    m_geom_cache.clear();
    m_contents_margins = m_effective_margins = Margins();
}
...
*/
bool BoxLayoutHfw::checkWidget(QWidget* widget) const
{
    if (Q_UNLIKELY(!widget)) {
        qWarning("QLayout: Cannot add a null widget to %s/%ls", metaObject()->className(),
                  qUtf16Printable(objectName()));
        return false;
    }
    if (Q_UNLIKELY(widget == parentWidget())) {
...
*/
bool BoxLayoutHfw::checkLayout(QLayout* other_layout) const
{
    ...
    if (Q_UNLIKELY(other_layout == this)) {
...
QWidgetItem* BoxLayoutHfw::createWidgetItem(const QLayout* layout, QWidget* widget)
{
    Q_UNUSED(layout);
...
QSpacerItem* BoxLayoutHfw::createSpacerItem(const QLayout* layout, int w, int h,
